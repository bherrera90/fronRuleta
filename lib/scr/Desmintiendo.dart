...
class DesmintendoQuiz extends StatefulWidget {
  const DesmintendoQuiz({super.key});

  @override
  _DesmintendoQuizState createState() => _DesmintendoQuizState();
}

class _DesmintendoQuizState extends State<DesmintendoQuiz>
  ...
  String _statusMessage = 'MITOS\nDESMENTIDOS';
  Color _statusColor = Colors.blue;

  @override
  void initState() {
    super.initState();
    print('🟣 WIDGET ABIERTO: DesmintendoQuiz (Mitos Desmentidos) - initState ejecutado');
  ...
  void _onCorrectAnswer() async {
    print('Desmintiendo: Respuesta correcta - AudioManager habilitado: ${AudioManager().isSoundEnabled}');
  ...
  void _onIncorrectAnswer() async {
    print('Desmintiendo: Respuesta incorrecta - AudioManager habilitado: ${AudioManager().isSoundEnabled}');
    // Usar el AudioManager que ya está configurado y respeta el estado global
  ...
  Future<void> _loadQuestionFromBackend() async {
    try {
      ...
      final categoryId = await RuletaApiService.getCategoryIdByName('Mitos Desmentidos');
      if (categoryId == null) {
        setState(() {
          _errorMessage = 'Categoría "Mitos Desmentidos" no encontrada';
    ...
    if (selectedText == _correctAnswer) {
      ...
      Future.delayed(const Duration(seconds: 3), () async {
        if (mounted) {
          ...
          // Capturamos el contexto del quiz antes de abrir el diálogo
          ...
          showDialog(
            ...
            builder: (dialogContext) => IncorrectAnswerDialog(
              ...
              onRetry: () async {
                ...
                // Regresa a la ruleta cerrando la pantalla del quiz.
      ...
      child: Column(
        children: [
          SizedBox(
            ...
            child: Image.asset(
              'assets/icons/icono-mitos desmentidos.png',
