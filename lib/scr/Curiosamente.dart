...
class CuriosamenteQuiz extends StatefulWidget {
  ...
  @override
  _CuriosamenteQuizState createState() => _CuriosamenteQuizState();
}

class _CuriosamenteQuizState extends State<CuriosamenteQuiz>
  ...
  String _statusMessage = 'CURIOSAMENTE';
  Color _statusColor = Colors.blue;

  @override
  void initState() {
    super.initState();
    print('🟢 WIDGET ABIERTO: CuriosamenteQuiz (Curiosamente) - initState ejecutado');
  ...
  void _onCorrectAnswer() async {
    print('Curiosamente: Respuesta correcta - AudioManager habilitado: ${AudioManager().isSoundEnabled}');
  ...
  void _onIncorrectAnswer() async {
    print('Curiosamente: Respuesta incorrecta - AudioManager habilitado: ${AudioManager().isSoundEnabled}');
    // Usar el AudioManager que ya está configurado y respeta el estado global
  ...
  Future<void> _loadQuestionFromBackend() async {
    try {
      ...
      final categoryId = await RuletaApiService.getCategoryIdByName('Curiosamente Mental');
      if (categoryId == null) {
        setState(() {
          _errorMessage = 'Categoría "Curiosamente Mental" no encontrada';
      ...
      child: Column(
        children: [
          SizedBox(
            ...
            child: Image.asset(
              'assets/icons/Icono-curiosamente.png',
