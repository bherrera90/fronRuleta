...
class QmasQuiz extends StatefulWidget {
  const QmasQuiz({super.key});

  @override
  _QmasQuizState createState() => _QmasQuizState();
}

class _QmasQuizState extends State<QmasQuiz>
  ...
  void initState() {
    super.initState();
    print('🟡 WIDGET ABIERTO: QmasQuiz (Q+ve) - initState ejecutado');
    print('🔊 Qmas: Estado del sonido al inicializar - AudioManager: ${AudioManager().isSoundEnabled}');
  ...
  void _onCorrectAnswer() async {
    print('Qmas: Respuesta correcta - AudioManager habilitado: ${AudioManager().isSoundEnabled}');
  ...
  void _onIncorrectAnswer() async {
    print('Qmas: Respuesta incorrecta - AudioManager habilitado: ${AudioManager().isSoundEnabled}');
    // Usar el AudioManager que ya está configurado y respeta el estado global
  ...
  Future<void> _loadQuestionFromBackend() async {
    try {
      ...
      final categoryId = await RuletaApiService.getCategoryIdByName('Q+VE');
      if (categoryId == null) {
        setState(() {
          _errorMessage = 'Categoría "Q+VE" no encontrada';
      ...
      child: Column(
        children: [
          SizedBox(
            ...
            child: Image.asset(
              'assets/icons/icono-amarrillo.png',
